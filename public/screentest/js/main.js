...
var parseSerializedForm = function (serializedArray) {
  const result = {};

  serializedArray.forEach(({ name, value }) => {
    // Usuń [] z nazw tablicowych pól (np. blackStars[])
    const key = name.replace(/\[\]$/, "");

    // Jeśli klucz już istnieje, zamień go w tablicę
    if (result[key]) {
  ...
  // Spróbuj sparsować clickCounter jeśli to JSON
  if (typeof result.clickCounter === "string") {
    try {
      result.clickCounter = JSON.parse(result.clickCounter);
    } catch (err) {
      // zostaw jako string jeśli nie parsuje się
    }
  }

  // Ujednolicenie nazwy focusTime → focustime
  if (result.focusTime && !result.focustime) {
    result.focustime = result.focusTime;
    delete result.focusTime;
  }
...
$(document).ready(function () {
  ...
  $("#frmscreenTestForm-submit").click(function (e) {
    ...
    $("#frm-screenTestForm input[name='clickCounter']").val(
      clickCounter_string
    );
    $("#frm-screenTestForm input[name='browser']").val(
    ...
    if ($("#smallestVisible input[type=radio]:checked").length < 1) {
      ...
      var data = $("#frm-screenTestForm").serializeArray();

      window.parent.postMessage(
        { type: "SCREENTEST_RESULT", payload: parseSerializedForm(data) },
        "*"
      );
    }
  });
